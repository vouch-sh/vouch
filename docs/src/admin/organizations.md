...
| **Remove** | Revokes the user's SSH certificates and then deletes the user record, cascading to their authenticators. Organization-scoped applications the user created move to another active administrator; their other applications are left without an owner. Audited as `admin_remove_user`. |
