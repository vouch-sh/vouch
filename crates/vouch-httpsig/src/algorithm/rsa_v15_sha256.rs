...
//! `rsa-v1_5-sha256` algorithm (RFC 9421 Section 3.3.2).
//!
//! Uses RSASSA-PKCS1-v1_5 with SHA-256 digest.
...
/// RSA PKCS#1 v1.5 SHA-256 signing key.
pub struct RsaV15Sha256Signer {
    key_pair: RsaKeyPair,
    key_id: String,
}

impl RsaV15Sha256Signer {
    ...
    #[must_use]
    pub fn verifier(&self) -> RsaV15Sha256Verifier {
        RsaV15Sha256Verifier {
...
impl std::fmt::Debug for RsaV15Sha256Signer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RsaV15Sha256Signer")
            .field("key_id", &self.key_id)
            .finish_non_exhaustive()
    }
}

impl SigningAlgorithm for RsaV15Sha256Signer {
    ...
    fn sign(&self, base: &[u8]) -> Result<Vec<u8>, HttpSigError> {
        ...
        self.key_pair
            .sign(&RSA_PKCS1_SHA256, &rng, base, &mut signature)
            .map_err(|e| HttpSigError::SigningFailed(format!("RSA-PKCS1 sign: {e}")))?;
        Ok(signature)
    }
}

/// RSA PKCS#1 v1.5 SHA-256 verification key.
#[derive(Debug, Clone)]
pub struct RsaV15Sha256Verifier {
    public_key: Vec<u8>,
    key_id: String,
}

impl RsaV15Sha256Verifier {
...
impl VerifyingAlgorithm for RsaV15Sha256Verifier {
    fn algorithm_id(&self) -> &str {
        "rsa-v1_5-sha256"
    }

    fn verify(&self, base: &[u8], signature: &[u8]) -> Result<(), HttpSigError> {
        let public_key = UnparsedPublicKey::new(&RSA_PKCS1_2048_8192_SHA256, &self.public_key);
        public_key
            .verify(base, signature)
            .map_err(|e| HttpSigError::VerificationFailed(format!("RSA-PKCS1 verify: {e}")))
    }
}
...
mod tests {
    ...
    fn test_sign_verify_roundtrip() {
        ...
        let signer = RsaV15Sha256Signer::from_pkcs8(&der, "test-key").unwrap();
    ...
    fn test_algorithm_id() {
        let der = test_rsa_pkcs8_key();
        let signer = RsaV15Sha256Signer::from_pkcs8(&der, "k1").unwrap();
        assert_eq!(signer.algorithm_id(), "rsa-v1_5-sha256");
        assert_eq!(signer.verifier().algorithm_id(), "rsa-v1_5-sha256");
    }

    #[test]
    fn test_key_id() {
        let der = test_rsa_pkcs8_key();
        let signer = RsaV15Sha256Signer::from_pkcs8(&der, "my-rsa-key").unwrap();
    ...
    fn test_signature_length() {
        let der = test_rsa_pkcs8_key();
        let signer = RsaV15Sha256Signer::from_pkcs8(&der, "k").unwrap();
    ...
    fn test_wrong_key_rejects() {
        ...
        let signer1 = RsaV15Sha256Signer::from_pkcs8(&der1, "k1").unwrap();
        let signer2 = RsaV15Sha256Signer::from_pkcs8(&der2, "k2").unwrap();
