...
mod tests {
    ...
    // =========================================================================
    // Cloud Integration Config Tests
    // =========================================================================
    ...
    fn test_aws_integration_config_without_role_arn() {
        ...
        // default_role_arn should be omitted from JSON
        assert!(!json.contains("default_role_arn"));

        let decoded: AwsIntegrationConfig = serde_json::from_str(&json).unwrap();
        assert!(decoded.default_role_arn.is_none());
    }
    ...
    fn test_integration_config_response_aws_configured() {
        ...
        let json = serde_json::to_string(&response).unwrap();
        let decoded: IntegrationConfigResponse<AwsIntegrationConfig> =
            serde_json::from_str(&json).unwrap();
        assert!(decoded.configured);
        let cfg = decoded.config.unwrap();
        assert_eq!(
