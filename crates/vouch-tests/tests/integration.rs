...
// ============================================================================
// Cloud Integration API Tests
// ============================================================================

mod cloud_integrations {
    use super::*;
    use vouch_common::{GcpIntegrationConfig, IntegrationConfigResponse};

    /// Test that GCP integration endpoint requires authentication.
    #[tokio::test]
    async fn test_gcp_integration_requires_auth() {
        let harness = TestHarness::new().await;

        let response = harness
            .get("/v1/integrations/gcp")
            .await
            .expect("Failed to get GCP integration");

        assert_eq!(response.status, 401);
    }

    /// Test that GCP integration requires org membership.
    #[tokio::test]
    async fn test_gcp_integration_requires_org() {
        let harness = TestHarness::new().await;

        // Create user without org membership
        let (_user, _auth_id, token) = harness
            .create_authenticated_user("noorg@example.com")
            .await
            .expect("Failed to create authenticated user");

        let response = harness
            .get_authenticated("/v1/integrations/gcp", &token)
            .await
            .expect("Failed to get GCP integration");

        // Should return 403 because user has no org
        assert_eq!(response.status, 403);
    }

    /// Test that org members can get GCP integration status.
    #[tokio::test]
    async fn test_get_gcp_integration_not_configured() {
        let harness = TestHarness::new().await;

        // Create org admin
        let (_user, _org, _auth_id, token) = harness
            .create_authenticated_org_admin("admin@testorg.com", "testorg.com")
            .await
            .expect("Failed to create org admin");
    ...
    /// Test that org admins can set GCP integration config.
    #[tokio::test]
    async fn test_set_gcp_integration_admin() {
        let harness = TestHarness::new().await;

        // Create org admin
        let (_user, _org, _auth_id, token) = harness
            .create_authenticated_org_admin("admin@setgcp.com", "setgcp.com")
            .await
            .expect("Failed to create org admin");
        ...
        let response = harness
            .put_json_authenticated("/v1/integrations/gcp", &config, &token)
            .await
            .expect("Failed to set GCP integration");

        assert_eq!(response.status, 200);

        let resp: GcpIntegrationConfig = response.json().expect("Failed to parse response");
        assert_eq!(resp.project_number, "123456789");
        assert_eq!(resp.pool_id, "vouch-pool");
        assert_eq!(resp.provider_id, "vouch-provider");
        assert_eq!(
            resp.service_account,
            Some("sa@project.iam.gserviceaccount.com".to_string())
        );

        // Verify it's saved by getting it back
        ...
        let get_resp: IntegrationConfigResponse<GcpIntegrationConfig> =
            get_response.json().expect("Failed to parse response");
        assert!(get_resp.configured);
        assert!(get_resp.config.is_some());
        let saved_config = get_resp.config.unwrap();
        assert_eq!(saved_config.project_number, "123456789");
    }

    /// Test that non-admins cannot set GCP integration config.
    #[tokio::test]
    async fn test_set_gcp_integration_non_admin_forbidden() {
        let harness = TestHarness::new().await;

        // Create org admin first to create the org
        let (_admin, org, _auth_id, _token) = harness
            .create_authenticated_org_admin("admin@nonadmin.com", "nonadmin.com")
            .await
            .expect("Failed to create org admin");

        // Create regular org member
        let (_member, _member_auth_id, member_token) = harness
            .create_authenticated_org_member("member@nonadmin.com", &org.id)
            .await
        ...
        let response = harness
            .put_json_authenticated("/v1/integrations/gcp", &config, &member_token)
            .await
            .expect("Failed to set GCP integration");
    ...
    /// Test GCP integration config validation.
    #[tokio::test]
    async fn test_set_gcp_integration_validation() {
        let harness = TestHarness::new().await;

        // Create org admin
        let (_user, _org, _auth_id, token) = harness
            .create_authenticated_org_admin("admin@validate.com", "validate.com")
            .await
            .expect("Failed to create org admin");

        // Empty project_number should fail
        let invalid_config = GcpIntegrationConfig {
        ...
        let response = harness
            .put_json_authenticated("/v1/integrations/gcp", &invalid_config, &token)
            .await
            .expect("Failed to set GCP integration");
    ...
    /// Test that org admins can delete GCP integration config.
    #[tokio::test]
    async fn test_delete_gcp_integration() {
        let harness = TestHarness::new().await;

        // Create org admin
        let (_user, _org, _auth_id, token) = harness
            .create_authenticated_org_admin("admin@delete.com", "delete.com")
            .await
            .expect("Failed to create org admin");
        ...
        // Delete it
        let response = harness
            .delete_authenticated("/v1/integrations/gcp", &token)
            .await
            .expect("Failed to delete GCP integration");
        ...
        let get_resp: IntegrationConfigResponse<GcpIntegrationConfig> =
            get_response.json().expect("Failed to parse response");
        assert!(!get_resp.configured);
        assert!(get_resp.config.is_none());
    }

    /// Test that deleting non-existent config returns 404.
    #[tokio::test]
    async fn test_delete_gcp_integration_not_found() {
        let harness = TestHarness::new().await;

        // Create org admin
        let (_user, _org, _auth_id, token) = harness
            .create_authenticated_org_admin("admin@notfound.com", "notfound.com")
            .await
            .expect("Failed to create org admin");
    ...
    /// Test that org members can read config set by admin.
    #[tokio::test]
    async fn test_org_member_can_read_gcp_config() {
        let harness = TestHarness::new().await;

        // Create org admin and set config
        let (_admin, org, _admin_auth_id, admin_token) = harness
            .create_authenticated_org_admin("admin@readable.com", "readable.com")
            .await
            .expect("Failed to create org admin");

        let config = GcpIntegrationConfig {
            project_number: "111222333".to_string(),
            pool_id: "member-pool".to_string(),
            provider_id: "member-provider".to_string(),
            service_account: Some("sa@example.iam.gserviceaccount.com".to_string()),
        };

        harness
            .put_json_authenticated("/v1/integrations/gcp", &config, &admin_token)
            .await
            .expect("Failed to set GCP integration");

        // Create regular org member
        let (_member, _member_auth_id, member_token) = harness
            .create_authenticated_org_member("member@readable.com", &org.id)
            .await
            .expect("Failed to create org member");

        // Member should be able to read the config
        let response = harness
            .get_authenticated("/v1/integrations/gcp", &member_token)
            .await
            .expect("Failed to get GCP integration");
        ...
        let resp: IntegrationConfigResponse<GcpIntegrationConfig> =
            response.json().expect("Failed to parse response");
        assert!(resp.configured);
        let saved_config = resp.config.expect("Config should exist");
        assert_eq!(saved_config.project_number, "111222333");
        assert_eq!(saved_config.pool_id, "member-pool");
    }
}
