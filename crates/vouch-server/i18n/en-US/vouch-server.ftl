...
apps-error-fapi-no-secrets = FAPI clients do not use client secrets.
...
apps-invalid-auth-method = Invalid client authentication method "{ $method }". Choose client_secret_basic or private_key_jwt.
apps-invalid-auth-method-confidential-required = Only web and service applications choose a client authentication method. Native and single-page applications are public clients.
apps-invalid-fapi-secret-auth = FAPI 2.0 applications authenticate with a private key (private_key_jwt), not a client secret.
apps-invalid-pkjwt-missing-jwks = Private key authentication (private_key_jwt) requires a JWKS or JWKS URI.
apps-invalid-pkjwt-jwks-algorithm = Private key authentication requires a JWKS key usable with ES256, RS256, PS256, or EdDSA for client-assertion signing. None of the configured keys qualify: each either declares an algorithm outside that set, has a key type the signing-key matcher cannot select for those algorithms, or is marked for a non-signing use. Add a compatible key, or adjust an existing key's alg, kty, or use.
...
apps-detail-standard-desc = Bearer tokens, with client secret or private key authentication.
...
apps-create-secprofile-standard-desc = Standard { -oauth } with bearer tokens. Authenticate with a client secret or a private key.
...
apps-create-auth-label = Client Authentication
apps-create-auth-secret = Client secret
apps-create-auth-secret-desc = Vouch issues a client secret, sent with client_secret_basic.
apps-create-auth-key = Private key (private_key_jwt)
apps-create-auth-key-desc = Sign client assertions with your key. No client secret is issued, and tokens stay bearer tokens.
apps-create-key-jwks-info = Provide the public keys your application signs client assertions with, as a { -jwks } (inline JSON) or a { -jwks } URI endpoint.
...
github-connect-link-body = Link your { -github } account before you connect an installation. { -product } checks that your account can access each installation it links.
...
appcreate-js-keyauth-required = Private key authentication requires either a { -jwks } or { -jwks } URI.
...
admin-audit-filter-invalid-since = "Since" is not a valid timestamp. Use RFC 3339 form, for example 2026-01-01T00:00:00Z.
admin-audit-filter-invalid-until = "Until" is not a valid timestamp. Use RFC 3339 form, for example 2026-12-31T00:00:00Z.
...
admin-scim-max = Maximum of { $max } API tokens reached. Revoke an existing token before creating a new one.
