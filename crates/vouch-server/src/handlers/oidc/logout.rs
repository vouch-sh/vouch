...
mod tests {
    ...
    // ================================================================
    //
    // The bug: `clear_user_session` built `ClientInfo::from(headers)`, whose
    // `From<&HeaderMap>` impl hard-codes `client_ip: None` (`extractors.rs`),
    // instead of using the `ClientInfo` request extractor that resolves the
    // peer IP from `ConnectInfo<SocketAddr>` + `resolve_client_ip`. The
    ...
    /// `geo::tests`. This is the resolvable case of G2 that the bug report
    /// could only infer (loopback yields no geo on either path).
    #[tokio::test]
