...
/// Step-6 client reassignment in `delete_user` must go through the OCC
/// version guard (`compare_and_update`), not a blind `update_by_index`:
/// `OAuthClientDoc`'s version is the serialization point for all secret-set
/// mutations (`update_oauth_client` / `update_oauth_client_registration`
/// write via `compare_and_update`), and a blind
/// `UPDATE ... version = version + 1` between a `find_all` read and the
/// write silently overwrites a concurrently-committed client update with
/// the stale doc — the same lost-update anomaly `delete_authenticator`'s
/// cascade fixed for `DeviceAuthRequestDoc`. A lost CAS is surfaced as
/// `DeleteUserError::OccConflict` and retried by the entry-point
/// `with_dsql_retry!` from a fresh read.
///
/// This sequential test pins the guarded path against a doc whose version
/// has already advanced past the initial insert (several prior committed
/// updates), asserting the reassignment lands on the LATEST doc state with
/// every non-`user_id` field intact. The true concurrent race (an update
/// committing between the cascade's read and write) needs a
/// multi-connection PostgreSQL harness — the SQLite in-memory backend
/// serialises writers, so the window does not open here (same limitation
/// as `test_delete_authenticator_preserves_consumed_device_auth_for_replay_revocation`).
#[tokio::test]
