...
//! Additional email-domain lifecycle for organizations.
//!
//! Add -> verify -> remove for non-primary email domains, plus the background
//! maintenance that keeps them honest: stale-pending cleanup and periodic
//! re-verification with the verified->unverified flip. Mutations are OCC
...
mod tests {
    ...
    async fn remove_additional_domain_revokes_matching_user_sessions() {
        use crate::db::documents::session::{SessionDoc, SessionPurpose};
        use crate::db::documents::user::UserDoc;
