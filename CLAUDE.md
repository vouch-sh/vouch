...
- **OAuth/OIDC protocol strings** — `error`, `error_description`, and the rest
  of the JSON API. RFC 6749 §5.2 addresses `error_description` to "the client
  developer" and restricts it to `%x20-21 / %x23-5B / %x5D-7E`, so it stays
  ASCII English. When one failure has both audiences, write two strings, not
  one: `AppValidationError::message()` for the API and
  `AppValidationError::localized()` for the page.
- **A protocol string interpolated into a translated sentence** — passed
  through verbatim as a Fluent placeable (`{ $detail }`). The sentence around
  it is translated; the protocol text is not.
- **`tracing` output** — English, always, including fatal startup errors. The
  audience is an operator reading logs, and the boundary is the subscriber,
  not how bad the error is.
- **Config and environment errors, clap built-ins, log lines.**
