...
The i18n layer is wired so adding a translation is purely a content change — no Rust touched per language.

1. Copy the catalog: `cp -r crates/vouch-server/i18n/en-US crates/vouch-server/i18n/<bcp47-tag>` (e.g. `fr-FR`).
2. Translate the values in the new `vouch.ftl`. Keep the same message IDs (the completeness test `every_template_key_is_defined` enforces parity with templates and `JS_I18N_KEYS`).
